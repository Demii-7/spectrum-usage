"""Train a tunable 1D model from scratch on all frequency bins across five seeds."""
...
def resolve_winner(search_path: str, model_name: str, filesystem) -> tuple[dict[str, Any], dict[str, Any]]:
    require_ray()
    ...
    return parameters, dict(winner.metrics)
...
def _test_metrics(config: dict[str, Any], model_name: str, run_directory: Path) -> dict[str, float]:
    ...
    test_metrics = metrics[metrics["split"].map(_is_test_split)]
...
def launch(args: argparse.Namespace) -> None:
    ...
    parameters, source_metrics = resolve_winner(args.search_path, args.model, filesystem)

    def trainable(values):
        seed = int(values["seed"])
        trial_directory = Path(tune.get_context().get_trial_dir())
        reporter = TuneReporter()
        run_integrated_trial(config, args.model, parameters, seed, trial_directory, reporter=reporter)
        test = _test_metrics(config, args.model, trial_directory)
    ...
    tuner = tune.Tuner(
        ...
        run_config=RunConfig(name=f"{args.model}-all-bins-train", storage_path=args.storage_path),
    )
    results = tuner.fit()
