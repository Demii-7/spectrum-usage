"""Launch independent zero-shot 2D transfer tasks on Ray with MinIO artifacts."""
...
from training.common.transfer_2d import (
    ROOT, SEED, TRANSFER_MODELS, evaluate_transfer_model, load_target,
    load_transfer_manifest, manifest_digest, preflight_checkpoints, repository_path,
    resolve_checkpoint, transfer_tasks,
)
...
def parse_args(argv=None):
    ...
    parser.add_argument("--manifest", type=Path,
                        default=ROOT / "training/configs/transfer_2d_manifest.json")
    ...
    parser.add_argument("--model", action="append", dest="model_names", default=None,
    ...
    parser.add_argument("--prefix", default="spectrum-usage/transfer-2d")
...
def _run_task(task, entry, models, model_names, config_path: str, bucket: str, prefix: str, digest: str, seed: int):
    client = _s3_client()
    ...
    source = load_target(task, [repository_path(value) for value in entry["files"]],
                         impute=bool(entry.get("impute", True)),
                         max_missing_gap=int(entry.get("max_missing_gap", 10)))
    with tempfile.TemporaryDirectory(prefix=f"transfer-{task.task_id}-") as temporary:
        ...
        for model_name in model_names:
            ...
            all_metrics.extend(evaluate_transfer_model(
                task=task, source=source, model_name=model_name,
                checkpoint_path=resolve_checkpoint(models[model_name]), config=config,
                output_dir=model_dir, seed=seed,
            ))
        pd.DataFrame(all_metrics).to_csv(task_dir / "aggregate_metrics.csv", index=False)
        (task_dir / "complete.json").write_text(
                        json.dumps({"task_id": task.task_id, "seed": seed,
                        "manifest_sha256": digest}, sort_keys=True) + "\n", encoding="utf-8")
        client.put_object(Bucket=bucket, Key=key, Body=_zip_directory(task_dir),
...
def launch(args):
    ...
    model_paths = {
        name: path.replace("seed-42", f"seed-{args.seed}")
    ...
    preflight_checkpoints(
        config, {name: resolve_checkpoint(path) for name, path in model_paths.items()}
    )
    ...
    selected = [all_tasks[task_id] for task_id in args.task_ids] if args.task_ids else list(all_tasks.values())
    ...
    remote_task = ray.remote(num_cpus=1, num_gpus=0.5,
                             max_retries=1 if args.retry_failed else 0)(_run_task)
    pending, results = [], []
    limit = args.max_concurrent or len(selected)
    for task in selected:
        pending.append(remote_task.remote(
            task, manifest["tasks"][task.task_id], model_paths, model_names, manifest["config"],
            args.bucket, args.prefix, digest, args.seed,
        ))
    ...
    (args.output_dir / "transfer_manifest.json").write_text(
        json.dumps({"schema_version": 1, "seed": args.seed, "manifest_sha256": digest,
                    "bucket": args.bucket, "prefix": args.prefix, "results": results},
