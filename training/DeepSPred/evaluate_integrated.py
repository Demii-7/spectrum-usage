...
def evaluate_chunk(config: dict[str, Any], chunk, bands, out: Path, checkpoint_path: Path):
    dcfg = build_runner_config(config, 1)["train"]
    ...
    runner = ckpt.get("resolved_config", ckpt["model_config"])
    if int(runner["frames"]["w_orig"]) != train_raw.shape[1]:
        raise ValueError("Checkpoint width does not match evaluation data")
    checkpoint_frequencies = ckpt.get("frequencies")
    if checkpoint_frequencies is not None and not np.allclose(
        np.asarray(checkpoint_frequencies), np.asarray(data.frequencies)
    ):
    ...
    if max(horizons) > frame_height * output_frames:
    ...
    for split_name in test_splits:
        ...
        normalized = _normalize(split_raw, stats["vmin"], stats["vmax"])
        rgb = _colormap(normalized, runner["preprocessing"]["colormap"])
        frame_pad = _pad_w(
            _make_frames(rgb, frame_height), runner["frames"]["w_pad"]
        )
        ...
        pred_scalar = invert_colormap(
            pred_rgb.transpose(0, 1, 3, 4, 2),
            cmap_name=str(ckpt.get("colormap", ckpt["model_config"]["preprocessing"]["colormap"])),
        )
