...
The first integrated pass keeps DeepSPred on shared CSV chunk data and reuses the model's RGB spectrogram frame pipeline:
...
Integrated outputs are written to `training/results/DeepSPred/` with metrics tables, per-chunk timing logs, `report.txt`, and `checkpoints/`.
