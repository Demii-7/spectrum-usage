...
Standalone TSS-LCD adaptation for the AERPAW spectrum prediction pipeline. The **paper** (IEEE TCCN 2026) is the primary architecture reference. Where the original repository differs from the paper, those differences are documented separately under *Repo-Specific Implementation Notes*.

## Quick Start

## Integrated Pipeline Usage

Use the integrated runner for the branch-wide chunk pipeline:
...
- **Note:** The exact Conv1D U-Net skip-connection implementation was derived from the repository (`NoiseNet.py`) and should still be validated against the paper if this model is revised again.
...
The Stage 2 objective in this standalone implementation uses the documented `projection_to_latent` bridge by default. Individual TSS branches can be toggled via config for ablation.
...
The paper's Fig. 6 also describes a Conv1D U-Net but specific channel counts and skip-connection topology may differ. **If this model is revised again, re-check the skip wiring against the paper's Fig. 6.**
...
6. **Stage 2 objective**: The paper does not specify an explicit loss between `H_fusion` and `z0`; their shapes differ. The standalone implementation uses `projection_to_latent`, but this should be revisited if the model is refactored.
