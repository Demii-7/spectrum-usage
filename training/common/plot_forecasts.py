...
def plot_mae_rmse_vs_horizon(results_dir: Path, out_dir: Path):
...
def plot_spectrograms(results_dir: Path, out_dir: Path, meta: dict,
    ...
    for col, horizon in enumerate(SPECTROGRAM_HORIZONS):
        ...
        pred_norm = pred_npz[key][:max_steps] * train_std_2d + train_mean_2d
        target = target_npz[key][:max_steps].astype(np.float64)
...
def plot_r2_vs_frequency(results_dir: Path, out_dir: Path, band_id: str, chunk_label: str,
    ...
    for h in HORIZONS:
        key = f"t_plus_{h}"
        pred = (pred_npz[key] * train_std + train_mean).astype(np.float32)
        tgt = target_npz[key].astype(np.float32)
...
def plot_time_series(results_dir: Path, out_dir: Path, band_id: str, chunk_label: str,
    ...
    gt_tgt = target_npz[gt_key][:max_steps].astype(np.float32)
    ...
    for idx, bin_idx in enumerate(bins):
        ...
        for h in HORIZONS:
            ...
            pred = (pred_npz[key][:max_steps, bin_idx] * train_std[bin_idx] + train_mean[bin_idx]).astype(np.float32)
...
def generate_all_plots(
    ...
    plot_mae_rmse_vs_horizon(results_dir, out_dir)
    ...
    plot_mae_vs_frequency(results_dir, out_dir, band_id, chunk_label)
