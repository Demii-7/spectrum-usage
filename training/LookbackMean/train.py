"""Train the LookbackMean baseline on POWDER spectrum data."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LookbackMean baseline.")
...
def main() -> None:
    ...
    model = LookbackMeanForecaster(build_cfg).to(device)
    ...
    do_early_stop = bool(train_cfg.get("early_stopping", False))
    epochs = int(train_cfg["epochs"])

    model_name = f"lookbackmean{args.mode}"
