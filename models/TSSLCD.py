...
def cosine_beta_schedule(T: int, s: float = 0.008) -> np.ndarray:
    """Cosine noise schedule (cosine² ᾱ from Nichol & Dhariwal).
...
class DiffusionModel(nn.Module):
    ...
    def __init__(self, latent_dim: int, n_timestep: int, device: torch.device,
        ...
        # Precompute β, α, ᾱ for all timesteps
    ...
    def q_sample(self, z0: torch.Tensor, t: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        """Forward diffusion: sample z_t = √ᾱ_t · z0 + √(1-ᾱ_t) · ε."""
    ...
    def p_sample_loop(self, cond_z: torch.Tensor) -> torch.Tensor:
        ...
        B = cond_z.size(0)
        z = torch.randn(B, self.latent_dim, device=device, dtype=cond_z.dtype)
...
# =====================================================================
#  Shared-pipeline inference adapter
# =====================================================================

class TSSLCDForecaster(nn.Module):
    """Composite TSS-LCD shared-pipeline model."""
    ...
    def __init__(
        ...
        *,
        ...
        self.condition_constructor = condition_constructor
    ...
    def forward(
        ...
        if x.ndim != 3:
            raise ValueError(
                "TSS-LCD expects input shaped "
                "(batch, lookback, frequencies), "
                f"got {tuple(x.shape)}."
            )

        batch, lookback, feature_count = x.shape

        if lookback != self.input_sequence_length:
            raise ValueError(
                "TSS-LCD received lookback "
                f"{lookback}, but was configured "
                f"for {self.input_sequence_length}."
            )

        if feature_count != self.input_size:
            raise ValueError(
                "TSS-LCD received "
                f"{feature_count} features, but "
                f"was configured for "
                f"{self.input_size}."
            )

        condition_latent = self.condition_constructor(x)

        generated_latent = self.diffusion.p_sample_loop(
            condition_latent
        )

        output = self.decoder(generated_latent)
